          ...
          <div className="container my-3">
            <div className="row my-4 d-flex justify-content-evenly" >
              {articles.map((element, index) => {
                return <div className="col-md-4 mx-36" key={index}>
                  <NewItem title={element.title ? element.title.slice(0, 45) : ""} description={element.description ? element.description.slice(0, 80) : ""} imgUrl={element.urlToImage} newsUrl={element.url} author={element.author} date={element.publishedAt} source={element.source.name} />
                </div>
              })}
            </div>
          </div>
